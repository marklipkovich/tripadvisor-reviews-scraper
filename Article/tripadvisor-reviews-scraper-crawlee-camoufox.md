...
My first implementation used standard Playwright with Chromium and Patchright (a stealth-patched Chromium fork). On the first local run, I got this captcha:
...
The `geoip` launch option (probed exit IP string) and how it fits with Crawlee’s **context-level** proxy injection are explained in detail in [§7 Anti-blocking measures](#7-anti-blocking-measures).
...
### Match browser timezone and geolocation to the proxy's exit IP (GeoIP)

Every browser fingerprint includes timezone, locale, and geolocation. For DataDome, **timezone and geolocation** especially need to line up with where the proxy’s exit IP actually is. A residential IP in Brazil with a default/European timezone (or none) reads as inconsistent. If they don't match, DataDome is more likely to challenge even when the rest of the fingerprint looks clean.
...
The fix is to align the browser’s **timezone and geolocation** with the proxy’s actual exit IP before pages load. (This Actor still passes **`locale="en-US"`** in Camoufox launch options for consistent English Intl APIs; the big win was fixing **time zone / coordinates vs egress IP**, not pretending the UI language is Portuguese.)

Camoufox’s **`geoip` parameter** accepts a plain IP address string; Camoufox looks it up in its **local MaxMind** database to set geolocation and timezone. The network work happens before the browser opens: an **httpx** call through the proxy URL gives you the exit IP to pass in.

Camoufox also accepts **`geoip=True`**, which resolves the exit IP inside the library (if you pass a **`proxy`** dict into Camoufox launch, that resolution goes through that proxy). I don’t use that path here — the Crawlee stack injects proxy at **Playwright context** creation, not on Camoufox’s launch kwargs, so probing with **httpx** first and passing the plain IP string is the cleaner fit.

The approach: probe the exit IP with httpx before the browser opens, then pass the result to Camoufox:
...
After passing the probed exit IP into `launch_options["geoip"]` at browser launch, **timezone and geolocation** follow the residential egress IP (via MaxMind). Combined with the same proxy session on the Playwright context, the "Captcha detected" loop I saw on timezone mismatch stopped showing up in the logs.
...
To verify this, I logged the proxy IP and browser session for each place. That's when I found a subtle Crawlee issue that was silently defeating the strategy.
...
The fix is to increase the browser inactive threshold and the inactive interval (abbreviated code):
...
**Keep the same proxy and browser session for all places — unless you're actually blocked.** Every scraping guide says "rotate your proxy." For DataDome, rotating on every place is counterproductive. A browser that's passed the challenge carries approval cookies forward to the next place. Rotating resets all that accumulated trust and forces the full challenge to run again on a cold IP. The default `browser_inactive_threshold` / interval behavior was silently defeating this strategy for me — the browser was being replaced every 45 seconds without any obvious error in the logs.

**Pass the probed exit IP into `geoip` at browser launch.** Camoufox needs a target IP string so it can align **timezone and geolocation** with that egress (via local MaxMind). Rather than **`geoip=True`** plus a Camoufox **`proxy`** launch dict (the library’s own IP discovery path), I probe first with **httpx** through the same proxy URL Crawlee uses, then set `launch_options["geoip"] = probe_ip`.

You can find the full Actor code in the [GitHub repository](https://github.com/marklipkovich/tripadvisor-reviews-scraper) and the deployed version on the [Apify Store](https://apify.com/marklp/tripadvisor-reviews-scraper). Questions or improvements? Join the [Discord](https://discord.com/invite/jyEM2PRvMU) — 11,000+ developers working through exactly these kinds of problems.
