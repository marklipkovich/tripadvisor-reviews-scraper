...
class CamoufoxPlugin(PlaywrightBrowserPlugin):
    ...
    returns the current session's proxy URL.  When provided:
      • _probe_proxy_timezone(proxy_url) runs over httpx to learn the exit IP;
        that plain IP string is passed to Camoufox as launch_options["geoip"]
        (not the proxy URL). Camoufox looks it up in MaxMind and sets timezone,
        geolocation, etc. — reducing IP vs fingerprint mismatch for DataDome.
      • Probe runs before AsyncNewBrowser; results are cached in browser_state
        for logging in handle_place without a second probe.
    """
